...
export function StudentsProvider ({children}) {
    ...
    const createStudent = async (data) => {
        ...
        try{
            ...
            const response = await axios.post(`${URL}api/v1//api/v1/students/${idUser}`,
            {
                "cpf": cpf,
                "bornYear":bornYear,
                "firstName": firstName,
    ...
    return (
        <StudentsContext.Provider value={{listStudent, listStudents, createStudent, resultCreateStudent}}>{children}</StudentsContext.Provider>
    )
}